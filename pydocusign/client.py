...
from pydocusign import exceptions,models
...
class DocuSignClient(object):
    ...
    def update_envalope(self, envelopeId):
        if not self.account_url:
            self.login_information()
        url = '/accounts/{accountId}/envelopes/{envelopeId}' \
              .format(accountId=self.account_id,
    ...
    def creatingenvelope(self):
        if not self.account_url:
            self.login_information()
        url = '/accounts/{accountId}/envelopes' \
              .format(accountId=self.account_id)
        response_data = self._request(
            url, method='POST', expected_status_code=201)
    ...
    def create_correct(self,envelopeId=''):
        if not self.account_url:
            self.login_information()
